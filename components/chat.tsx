...
export function Chat({
  ...
  const handleWebSearch = useCallback(() => {
    if (!input?.trim()) {
      toast({ type: 'error', description: 'Please enter a query to search the web.' });
      return;
    }

    // Submit current input so the model can run and (if capable) call the search tool.
    // We pass current attachments as before.
